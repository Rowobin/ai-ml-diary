...
On day 3, I studied the inner workings of a Neural Network. I briefly explored the concepts of
forward propagation, loss functions and backward propagation. Today, I decided to further explore these concepts by implementing a model capable of performing a linear regression.
...
- **The losses are not normalized:** they are in the scale of the data, which means a larger slope leads to higher variance in the y value. Losses can only be compared when using the same dataset, or when using different datasets that have been normalized to the same numeric range.

- **The closer the slope is to zero, the smaller the effect of x on y:** this means the model has less information about y. Deep Learning models learn by observing the relationships between variables, the smaller the relation, the worse the model.
